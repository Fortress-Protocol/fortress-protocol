...
async function setPrice(world: World, from: string, priceOracle: PriceOracle, fToken: string, amount: NumberV): Promise<World> {
  return addAction(
    world,
    `Set price oracle price for ${fToken} to ${amount.show()}`,
    await invoke(world, priceOracle.methods.setUnderlyingPrice(fToken, amount.encode()), from)
  );
}
...
export function priceOracleCommands() {
  return [
    ...
    new Command<{priceOracle: PriceOracle, fToken: AddressV, amount: NumberV}>(`
        #### SetPrice
      ...
      [
        ...
        new Arg("fToken", getAddressV),
        new Arg("amount", getExpNumberV)
      ],
      (world, from, {priceOracle, fToken, amount}) => setPrice(world, from, priceOracle, fToken.val, amount)
    ),

    new Command<{priceOracle: PriceOracle, address: AddressV, amount: NumberV}>(`
        #### SetDirectPrice

        * "SetDirectPrice <Address> <Amount>" - Sets the per-bnb price for the given fToken
