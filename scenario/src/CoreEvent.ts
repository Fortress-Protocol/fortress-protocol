...
import { fTokenCommands, processFTokenEvent } from './Event/FTokenEvent';
import { fTokenDelegateCommands, processFTokenDelegateEvent } from './Event/FTokenDelegateEvent';
...
import { ftsCommands, processFTSEvent } from './Event/FTSEvent';
import { faiCommands, processFAIEvent } from './Event/FAIEvent';
...
import { FortressLens } from './Contract/FortressLens';
...
export const commands: (View<any> | ((world: World) => Promise<View<any>>))[] = [
  ...
  new Command<{ event: EventV }>(
    `
      ...
      * "Trx ...trxEvent" - Handles event to set details of next transaction
        * E.g. "Trx Value 1.0e18 (FToken fBnb Mint 1.0e18)"
    `,
    'Trx',
  ...
  new Command<{ event: EventV }>(
    `
      ...
      * "Expect ...expectation" - Adds an expectation to hold after the next transaction
        * E.g. "Expect Changes (FToken fZRX TotalSupply) +10.0e18"
    `,
    'Expect',
    [new Arg('event', getEventV, { variadic: true })],
  ...
  new Command<{ given: Value; event: EventV }>(
    `
      ...
      * "Given value event" - Runs event only if value is truthy. Thus, given can be used to build existence checks.
        * E.g. "Given ($var) (PriceOracle SetPrice fBAT $var)"
    `,
    'Given',
  ...
  new Command<{ event: EventV }>(
    `
      ...
      * "FToken ...event" - Runs given FToken event
        * E.g. "FToken fZRX Mint 5e18"
    `,
    'FToken',
    [new Arg('event', getEventV, { variadic: true })],
    (world, from, { event }) => processFTokenEvent(world, event.val, from),
    { subExpressions: fTokenCommands() }
  ),
  ...
  new Command<{ event: EventV }>(
    `
      ...
      * "FTokenDelegate ...event" - Runs given FTokenDelegate event
        * E.g. "FTokenDelegate Deploy FDaiDelegate fDaiDelegate"
    `,
    'FTokenDelegate',
    [new Arg('event', getEventV, { variadic: true })],
    (world, from, { event }) => processFTokenDelegateEvent(world, event.val, from),
    { subExpressions: fTokenDelegateCommands() }
  ),
  ...
  new Command<{ event: EventV }>(
    `
      ...
      * E.g. "PriceOracleProxy Deploy (Unitroller Address) (PriceOracle Address) (FToken fBNB Address)"
    `,
    'PriceOracleProxy',
    [new Arg('event', getEventV, { variadic: true })],
    (world, from, { event }) => {
  ...
  new Command<{ event: EventV }>(
    `
      ...
      * E.g. "Maximillion Deploy (FToken fBNB Address)"
    `,
    'Maximillion',
    [new Arg('event', getEventV, { variadic: true })],
    (world, from, { event }) => {
  ...
  new Command<{ event: EventV }>(
    `
      ...
      * "FTS ...event" - Runs given fts event
      * E.g. "FTS Deploy"
    `,
    'FTS',
    [new Arg('event', getEventV, { variadic: true })],
    (world, from, { event }) => {
      return processFTSEvent(world, event.val, from);
    },
    { subExpressions: ftsCommands() }
  ),
  ...
  new Command<{ event: EventV }>(
    `
      ...
      * "FAI ...event" - Runs given fts event
      * E.g. "FAI Deploy"
    `,
    'FAI',
    [new Arg('event', getEventV, { variadic: true })],
    (world, from, { event }) => {
      return processFAIEvent(world, event.val, from);
    },
    { subExpressions: faiCommands() }
  ),
  ...
  buildContractEvent<FortressLens>("FortressLens", false),
