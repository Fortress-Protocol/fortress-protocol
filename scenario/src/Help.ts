...
export function printHelp(printer: Printer, event: Event, expressions: Expression<any>[], path: string[]=[]) {
  if (event.length === 0) {
    ...
    if (path.length === 0) {
      banner = (
`
## Fortress Command Runner

The Fortress Command Runner makes it easy to interact with Fortress. You can input simple commands
...
sub-commands run \`Help FToken\` or \`Help FToken Mint\`.
`).trim();
    } else {
