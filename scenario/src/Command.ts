        ...
        if (eventArg === undefined) {
          ...
          try {
            ...
            if (arg.rescue) {
              ...
              // `Gate (FToken Bnb Address) (... deploy fToken)`
              // could be used to deploy a fToken if it doesn't exist, but
              // since there is no FToken, that check would raise (when we'd
