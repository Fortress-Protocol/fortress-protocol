...
import { fTokenFetchers, getFTokenValue } from './Value/FTokenValue';
import { fTokenDelegateFetchers, getFTokenDelegateValue } from './Value/FTokenDelegateValue';
...
import { getFTSValue, ftsFetchers } from './Value/FTSValue';
import { getFAIValue, faiFetchers } from './Value/FAIValue';
...
const fetchers = [
    ...
    async (world, { addr, slot, key, nestedKey, valType }) => {
      ...
      switch (valType.val) {
        case 'marketStruct':
          ...
          let isFtsStr = await world.web3.eth.getStorageAt(addr.val, isCompKey);

          return new ListV([
            new BoolV(isListed),
            new ExpNumberV(collateralFactor.toString(), 1e18),
            new BoolV(areEqual(userInMarket, 1)),
            new BoolV(areEqual(isFtsStr, 1))
          ]);
        default:
          return new NothingV();
      }
  ...
  new Fetcher<{ given: Value; expected: Value }, BoolV>(
    `
      ...
      * "Equal given:<Value> expected:<Value>" - Returns true if given values are equal
        ...
        * E.g. "Equal (FToken fZRX TotalSupply) (Exactly 55)"
        * E.g. "Equal (FToken fZRX Comptroller) (Comptroller Address)"
    `,
    'Equal',
  ...
  new Fetcher<{ res: Value }, Value>(
    `
      ...
      * "FToken ...fTokenArgs" - Returns fToken value
    `,
    'FToken',
    [new Arg('res', getFTokenValue, { variadic: true })],
    async (world, { res }) => res,
    { subExpressions: fTokenFetchers() }
  ),
  new Fetcher<{ res: Value }, Value>(
    `
      #### FTokenDelegate

      * "FTokenDelegate ...fTokenDelegateArgs" - Returns fToken delegate value
    `,
    'FTokenDelegate',
    [new Arg('res', getFTokenDelegateValue, { variadic: true })],
    async (world, { res }) => res,
    { subExpressions: fTokenDelegateFetchers() }
  ),
  new Fetcher<{ res: Value }, Value>(
    `
      #### Bep20
  ...
  new Fetcher<{ res: Value }, Value>(
    `
      ...
      * "FTS ...ftsArgs" - Returns FTS value
    `,
    'FTS',
    [new Arg('res', getFTSValue, { variadic: true })],
    async (world, { res }) => res,
    { subExpressions: ftsFetchers() }
  ),
  new Fetcher<{ res: Value }, Value>(
    `
      #### FAI

      * "FAI ...ftsArgs" - Returns FAI value
    `,
    'FAI',
    [new Arg('res', getFAIValue, { variadic: true })],
    async (world, { res }) => res,
    { subExpressions: faiFetchers() }
  ),
  new Fetcher<{ res: Value }, Value>(
    `
      #### Governor
...
let contractFetchers = [
  ...
  { contract: "FortressLens", implicit: false },
