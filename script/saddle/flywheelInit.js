...
usage:
	npx saddle script -n {network} flywheel:init {
		batch: number || null, // how many borrowrs to claim fts for at once. default 100 (~2.3M gas)
		stage: bool || null // use stage API? default false
		fTokens: string[] || null, // which borrowers to get. defaults to all fTokens
...
let getConfig = (configArgs) => {
	...
	let res = {
		fTokens: getArray(config, 'fTokens', false) || [],
...
let getFTokenAddresses = (fTokenArgs) => {
	let all = [
		'fUSDC',
		'fDAI',
		'fUSDT',
	...
	let list = fTokenArgs.length == 0 ? all : fTokenArgs;
	let map = {};
	for(let val of list) {
...
let filterInitialized = async (borrowersByFToken) => {
	let res = {}
	let batchSize = 75;
	console.log(`Calling fortressBorrowerIndex for borrowers in batches of ${batchSize}...\n`);
	for(let fTokenAddr of Object.keys(borrowersByFToken)) {
		let speed = await call(Comptroller, 'fortressSpeeds', [fTokenAddr]);
		if (Number(speed) != 0){
			for (let borrowerChunk of getChunks(borrowersByFToken[fTokenAddr], batchSize)) {
				try {
					let indices = await Promise.all(borrowerChunk.map(
						async(borrower) => {
							return await call(Comptroller, 'fortressBorrowerIndex',[fTokenAddr, borrower])
					}));
					let uninitialized = borrowerChunk.filter((borrower, i) => Number(indices[i]) == 0);
					res[fTokenAddr] = res[fTokenAddr] ? res[fTokenAddr].concat(uninitialized) : uninitialized;
				} catch(e) {
...
// {[ftokenAddr] : borrowers}
let filterBorrowers = (apiAccounts, fTokenList) => {
	return apiAccounts.reduce((acc, account) => {
		let validBorrowers = account.tokens.filter(
			(accountFToken) =>
				fTokenList.includes(accountFToken.address) &&
				accountFToken.borrow_balance_underlying.value > 0
		);
		for (let borrower of validBorrowers) {
			let ftokenAddr = borrower.address;
			acc[ftokenAddr] = acc[ftokenAddr]
				? acc[ftokenAddr].concat(account.address)
				: [account.address];
		}
		return acc;
	}, {});
...
let claimFortressBatch = async (borrowersByFToken, opts) => {
	for (let fTokenAddr of Object.keys(borrowersByFToken)) {
		let borrowers = borrowersByFToken[fTokenAddr];
		for (let chunk of getChunks(borrowers, opts.batch)) {
			if (chunk.length == 0) {
				console.log(`No borrowers to claim for ${fTokenAddr}`);
			} else {
				console.log(
					`Sending tx to claim ${fTokenAddr.toString()} borrows for ${JSON.stringify(
						chunk
					)}\n`
				);
				try {
					let tx = await send(Comptroller, 'claimFortress', [
						chunk,
						[fTokenAddr],
						true,
						false,
...
(async () => {
	let borrowersByFToken;
	let fTokenMap; // symbol => addrs
	let opts = getConfig(args[0]);
	if (network == 'development') {
		borrowersByFToken = getTestData();
	} else if (isKnownNetwork(network)) {
		let apiAccounts = opts.readFixture
			? await readFixture()
			: await accountRequest(network, opts);
		let fTokenAddresses = Object.values(getFTokenAddresses(opts.fTokens));
		borrowersByFToken = filterBorrowers(apiAccounts, fTokenAddresses);
		if (opts.writeFixture) await writeFixture(apiAccounts);
	} else {
		printUsage();
	}
	let unInit = await filterInitialized(borrowersByFToken);
	print('Uninitialized accounts before: ', unInit);

	await claimFortressBatch(unInit, opts);
	unInit = await filterInitialized(borrowersByFToken);
