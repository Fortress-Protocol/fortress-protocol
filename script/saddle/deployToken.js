...
(async function() {
  ...
  console.log(`Deploying fToken with ${JSON.stringify(conf)}`);
  ...
  let contract = await saddle.deploy('FBep20Immutable', deployArgs);
  ...
  if (env['VERIFY']) {
    ...
    await saddle.verify(bscscanApiKey, contract._address, 'FBep20Immutable', deployArgs, 0);
