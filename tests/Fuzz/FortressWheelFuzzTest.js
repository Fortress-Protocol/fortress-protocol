...
describe.skip('FortressWheelFuzzTest', () => {
  // This whole test is fake, but we're testing to see if our equations match reality.

  // First, we're going to build a simple simulator of the Fortress protocol
  ...
  let initialState = globals => {
    return {
      ...
      fortressSupplySpeed: new bn(1),
      fortressSupplyIndex: new bn(1),
      fortressSupplyIndexSnapshots: {},
      fortressSupplyIndexUpdatedBlock: globals.blockNumber,

      fortressBorrowSpeed: new bn(1),
      fortressBorrowIndex: new bn(1),
      fortressBorrowIndexSnapshots: {},
      fortressSupplyIndexUpdatedBlock: globals.blockNumber,

      fortressAccruedWithCrank: {}, // naive method, accruing all accounts every block
      fortressAccruedWithIndex: {}, // with indices

      activeBorrowBlocks: new bn(0), // # blocks with an active borrow, for which we expect to see fts distributed. just for fuzz testing.
  ...
  let trueUpFortress = (globals, state) => {
    state = accrueInterest(globals, state);

    state = Object.keys(state.fortressSupplyIndexSnapshots).reduce(
      (acc, account) => supplierFlywheelByIndex(globals, state, account),
      state
    );

    state = Object.keys(state.fortressBorrowIndexSnapshots).reduce(
  ...
  // manual flywheel loops through every account and updates fts accrued mapping
  ...
  let flywheelByCrank = (
    ...
    for (let [account, balance] of Object.entries(balances)) {
      if (isPositive(totalSupply)) {
        fortressAccruedWithCrank[account] = get(
          state.fortressAccruedWithCrank[account]
        ).plus(
          deltaBlocks
            .times(fortressSupplySpeed)
            .times(balance)
            .div(totalSupply)
        );
      }
    ...
    return {
      ...state,
      fortressAccruedWithCrank: fortressAccruedWithCrank,
    };
  };

  // real deal fts index flywheel™️
  let borrowerFlywheelByIndex = (globals, state, account) => {
    let {
      fortressBorrowSpeed,
      fortressBorrowIndex,
      fortressBorrowIndexSnapshots,
      fortressAccruedWithIndex,
      totalBorrows,
      borrowBalances,
      fortressBorrowIndexUpdatedBlock,
      borrowIndex,
      borrowIndexSnapshots
    } = state;

    let deltaBlocks = globals.blockNumber.minus(fortressBorrowIndexUpdatedBlock);
    if (isPositive(totalBorrows)) {
      let scaledTotalBorrows = totalBorrows.div(borrowIndex);
      fortressBorrowIndex = fortressBorrowIndex.plus(
        fortressBorrowSpeed.times(deltaBlocks).div(scaledTotalBorrows)
      );
    }

    let indexSnapshot = fortressBorrowIndexSnapshots[account];

    if (
      indexSnapshot !== undefined &&
      fortressBorrowIndex.isGreaterThan(indexSnapshot) &&
      ...
      fortressAccruedWithIndex[account] = get(fortressAccruedWithIndex[account]).plus(
        borrowBalanceNew
          .div(borrowIndex)
          .times(fortressBorrowIndex.minus(indexSnapshot))
      );
    }

    return {
      ...state,
      fortressBorrowIndexUpdatedBlock: globals.blockNumber,
      fortressBorrowIndex: fortressBorrowIndex,
      fortressBorrowIndexSnapshots: {
        ...state.fortressBorrowIndexSnapshots,
        [account]: fortressBorrowIndex
      }
    };
  };
  ...
  let supplierFlywheelByIndex = (globals, state, account) => {
    let {
      ...
      fortressAccruedWithIndex,
      totalSupply,
      fortressSupplyIndexUpdatedBlock
    } = state;

    let deltaBlocks = globals.blockNumber.minus(fortressSupplyIndexUpdatedBlock);

    if (isPositive(totalSupply)) {
      fortressSupplyIndex = fortressSupplyIndex.plus(
        fortressSupplySpeed.times(deltaBlocks).div(totalSupply)
      );
    }

    let indexSnapshot = fortressSupplyIndexSnapshots[account];
    if (indexSnapshot !== undefined) {
      // if had prev snapshot,  accrue some fts
      fortressAccruedWithIndex[account] = get(fortressAccruedWithIndex[account]).plus(
        balances[account].times(fortressSupplyIndex.minus(indexSnapshot))
      );
    }
    ...
    return {
      ...state,
      fortressSupplyIndexUpdatedBlock: globals.blockNumber,
      fortressSupplyIndex: fortressSupplyIndex,
      fortressSupplyIndexSnapshots: {
        ...state.fortressSupplyIndexSnapshots,
        [account]: fortressSupplyIndex
      },
      fortressAccruedWithIndex: fortressAccruedWithIndex
    };
  };
  ...
  // assert amount distributed by the crank is expected, that it equals # blocks with a supply * fts speed
  let crankCorrectnessInvariant = (globals, state, events, invariantFn) => {
    let expected = state.activeSupplyBlocks
      .times(state.fortressSupplySpeed)
      .plus(state.activeBorrowBlocks.times(state.fortressBorrowSpeed));

    let actual = Object.values(state.fortressAccruedWithCrank).reduce(
    ...
    invariantFn(
      ...
      expected,
      actual,
      `crank method distributed fts inaccurately`
    );
  };

  // assert fts distributed by index is the same as amount distributed by crank
  let indexCorrectnessInvariant = (globals, state, events, invariantFn) => {
    let expected = state.fortressAccruedWithCrank;
    let actual = state.fortressAccruedWithIndex;
    invariantFn(
      (expected, actual) => {
  ...
  let runEvents = (globals, initState, events) => {
    ...
    return trueUpFortress(globals, state);
  };
