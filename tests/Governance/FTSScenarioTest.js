...
describe('FTSScenario', () => {
  let root, accounts;
  let fts;

  beforeEach(async () => {
    [root, ...accounts] = saddle.accounts;
    fts = await deploy('FTSScenario', [root]);
  });
    ...
    [
      ...
      it(`with ${checkpoints} checkpoints, has ${expectedReads} reads`, async () => {
        ...
        while (remaining > 0) {
          ...
          await fts.methods.generateCheckpoints(amt, offset).send({from: root, gas: 200000000});
          remaining -= amt;
          offset += amt;
        }

        let result = await fts.methods.getPriorVotes(root, 1).send();
