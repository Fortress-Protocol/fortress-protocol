...
describe('GovernorAlpha#state/1', () => {
  let fts, gov, root, acct, delay, timelock;
  ...
  beforeAll(async () => {
    ...
    gov = await deploy('GovernorAlpha', [timelock._address, fts._address, root]);
    await send(timelock, "harnessSetAdmin", [gov._address])
    await send(fts, 'transfer', [acct, bnbMantissa(400000)]);
    await send(fts, 'delegate', [acct], { from: acct });
  });
  ...
  it("Canceled", async () => {
    await send(fts, 'transfer', [accounts[0], bnbMantissa(400000)]);
    await send(fts, 'delegate', [accounts[0]], { from: accounts[0] });
    await mineBlock()
    ...
    await send(fts, 'delegate', [root], { from: accounts[0] });
