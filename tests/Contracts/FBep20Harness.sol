...
import "../../contracts/FBep20Immutable.sol";
import "../../contracts/FBep20Delegator.sol";
import "../../contracts/FBep20Delegate.sol";
import "../../contracts/FDaiDelegate.sol";
import "./ComptrollerScenario.sol";

contract FBep20Harness is FBep20Immutable {
    ...
    function harnessRedeemFresh(address payable account, uint fTokenAmount, uint underlyingAmount) public returns (uint) {
        return super.redeemFresh(account, fTokenAmount, underlyingAmount);
    }
    ...
    function harnessLiquidateBorrowFresh(address liquidator, address borrower, uint repayAmount, FToken fTokenCollateral) public returns (uint) {
        (uint err,) = liquidateBorrowFresh(liquidator, borrower, repayAmount, fTokenCollateral);
        return err;
    }
...
contract FBep20Scenario is FBep20Immutable {
    constructor(address underlying_,
                ComptrollerInterface comptroller_,
                InterestRateModel interestRateModel_,
                uint initialExchangeRateMantissa_,
...
contract FEvil is FBep20Scenario {
    constructor(address underlying_,
                ComptrollerInterface comptroller_,
                InterestRateModel interestRateModel_,
                uint initialExchangeRateMantissa_,
    ...
    function evilSeize(FToken treasure, address liquidator, address borrower, uint seizeTokens) public returns (uint) {
        return treasure.seize(liquidator, borrower, seizeTokens);
    }
}

contract FBep20DelegatorScenario is FBep20Delegator {
    constructor(address underlying_,
                ComptrollerInterface comptroller_,
                InterestRateModel interestRateModel_,
                uint initialExchangeRateMantissa_,
...
contract FBep20DelegateHarness is FBep20Delegate {
...
contract FBep20DelegateScenario is FBep20Delegate {
    constructor() public {}
...
contract FBep20DelegateScenarioExtra is FBep20DelegateScenario {
...
contract FDaiDelegateHarness is FDaiDelegate {
    uint blockNumber = 100000;
    uint harnessExchangeRate;
    bool harnessExchangeRateStored;
...
contract FDaiDelegateScenario is FDaiDelegate {
    function setTotalBorrows(uint totalBorrows_) public {
        totalBorrows = totalBorrows_;
    }
...
contract FDaiDelegateMakerHarness is PotLike, VatLike, GemLike, DaiJoinLike {
    /* Pot */
