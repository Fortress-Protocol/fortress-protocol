...
describe('Spinarama', () => {
  ...
  describe('#mintMint', () => {
    it('should succeed', async () => {
      ...
      await send(fToken.underlying, 'harnessSetBalance', [from, 100], {from});
      await send(fToken.underlying, 'approve', [fToken._address, '0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF'], {from});
      await minerStop();
      const p1 = send(fToken, 'mint', [1], {from});
      const p2 = send(fToken, 'mint', [2], {from});
      await minerStart();
      expect(await p1).toSucceed();
      expect(await p2).toSucceed();
      expect(await balanceOf(fToken, from)).toEqualNumber(3);
    });

    it('should partial succeed', async () => {
      const fToken = await makeFToken({supportMarket: true, underlyingPrice: 1});
      await send(fToken.underlying, 'harnessSetBalance', [from, 100], {from});
      await send(fToken.underlying, 'approve', [fToken._address, 10], {from});
      await minerStop();
      const p1 = send(fToken, 'mint', [11], {from});
      const p2 = send(fToken, 'mint', [10], {from});
      ...
      expect(await balanceOf(fToken, from)).toEqualNumber(10);
    });
  });
  ...
  describe('#mintRedeem', () => {
    it('should succeed', async () => {
      ...
      await send(fToken.underlying, 'approve', [fToken._address, 10], {from});
      await minerStop();
      const p1 = send(fToken, 'mint', [10], {from});
      const p2 = send(fToken, 'redeemUnderlying', [10], {from});
      await minerStart();
      expect(await p1).toSucceed();
      expect(await p2).toSucceed();
      expect(await balanceOf(fToken, from)).toEqualNumber(0);
    });
  });
  ...
  describe('#redeemMint', () => {
    it('should succeed', async () => {
      const fToken = await makeFToken({supportMarket: true, underlyingPrice: 1});
      await send(fToken, 'harnessSetTotalSupply', [10]);
      await send(fToken, 'harnessSetExchangeRate', [bnbMantissa(1)]);
      await send(fToken, 'harnessSetBalance', [from, 10]);
      await send(fToken.underlying, 'harnessSetBalance', [fToken._address, 10]);
      await send(fToken.underlying, 'approve', [fToken._address, 10], {from});
      await minerStop();
      const p1 = send(fToken, 'redeem', [10], {from});
      const p2 = send(fToken, 'mint', [10], {from});
      await minerStart();
      expect(await p1).toSucceed();
      expect(await p2).toSucceed();
  ...
  describe('#repayRepay', () => {
    it('should succeed', async () => {
      const fToken1 = await makeFToken({supportMarket: true, underlyingPrice: 1, collateralFactor: .5});
      const fToken2 = await makeFToken({supportMarket: true, underlyingPrice: 1, comptroller: fToken1.comptroller});
      await send(fToken1.underlying, 'harnessSetBalance', [from, 10]);
      await send(fToken1.underlying, 'approve', [fToken1._address, 10], {from});
      await send(fToken2.underlying, 'harnessSetBalance', [fToken2._address, 10]);
      await send(fToken2, 'harnessSetTotalSupply', [100]);
      await send(fToken2.underlying, 'approve', [fToken2._address, 10], {from});
      await send(fToken2, 'harnessSetExchangeRate', [bnbMantissa(1)]);
      expect(await enterMarkets([fToken1, fToken2], from)).toSucceed();
      expect(await send(fToken1, 'mint', [10], {from})).toSucceed();
      expect(await send(fToken2, 'borrow', [2], {from})).toSucceed();
      await minerStop();
      const p1 = send(fToken2, 'repayBorrow', [1], {from});
      const p2 = send(fToken2, 'repayBorrow', [1], {from});
      await minerStart();
      expect(await p1).toSucceed();
      expect(await p2).toSucceed();
      expect((await borrowSnapshot(fToken2, from)).principal).toEqualNumber(0);
    });
