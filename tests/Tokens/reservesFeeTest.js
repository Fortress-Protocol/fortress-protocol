...
describe('FToken', function () {
  let root, treasuryGuardian, treasuryAddress, accounts;
  beforeEach(async () => {
  ...
  describe('_setReserveFactorFresh', () => {
    let fToken;
    beforeEach(async () => {
      fToken = await makeFToken({comptrollerOpts: {kind: 'boolFee'}});
    });
  ...
  describe("_transferReservesFresh", () => {
    ...
    it("fails if called by non-admin", async () => {
      expect(
        await send(fToken, 'harnessTransferReservesFresh', [reduction], {from: accounts[0]})
    ...
    it("fails if market not fresh", async () => {
      expect(await send(fToken, 'harnessFastForward', [5])).toSucceed();
      expect(await send(fToken, 'harnessTransferReservesFresh', [reduction], {from: treasuryGuardian})).toHaveTokenFailure('MARKET_NOT_FRESH', 'TRANSFER_RESERVES_FRESH_CHECK');
      expect(await call(fToken, 'totalReserves')).toEqualNumber(reserves);
    });

    it("fails if amount exceeds reserves", async () => {
      expect(await send(fToken, 'harnessTransferReservesFresh', [reserves.add(1)], {from: treasuryGuardian})).toHaveTokenFailure('BAD_INPUT', 'TRANSFER_RESERVES_VALIDATION');
      expect(await call(fToken, 'totalReserves')).toEqualNumber(reserves);
    });
    ...
    it("fails if amount exceeds available cash", async () => {
      ...
      expect(await send(fToken, 'harnessTransferReservesFresh', [reserves], {from: treasuryGuardian})).toHaveTokenFailure('TOKEN_INSUFFICIENT_CASH', 'TRANSFER_RESERVES_CASH_NOT_AVAILABLE');
      expect(await call(fToken, 'totalReserves')).toEqualNumber(reserves);
    });

    it("increases admin balance and reduces reserves on success", async () => {
      const balance = bnbUnsigned(await call(fToken.underlying, 'balanceOf', [treasuryAddress]));
      expect(await send(fToken, 'harnessTransferReservesFresh', [reserves], {from: treasuryGuardian})).toSucceed();
      expect(await call(fToken.underlying, 'balanceOf', [treasuryAddress])).toEqualNumber(balance.add(reserves));
      expect(await call(fToken, 'totalReserves')).toEqualNumber(0);
    });

    it("emits an event on success", async () => {
      const result = await send(fToken, 'harnessTransferReservesFresh', [reserves], {from: treasuryGuardian});
  ...
  describe("_transferReserves", () => {
    ...
    it("emits a reserve-reduction failure if interest accrual fails", async () => {
      await send(fToken.interestRateModel, 'setFailBorrowRate', [true]);
      await fastForward(fToken, 1);
      await expect(send(fToken, '_transferReserves', [reduction], {from: treasuryGuardian})).rejects.toRevert("revert INTEREST_RATE_MODEL_ERROR");
    });

    it("returns error from _transferReservesFresh without emitting any extra logs", async () => {
      const {reply, receipt} = await both(fToken, 'harnessTransferReservesFresh', [reserves.add(1)], {from: treasuryGuardian});
      expect(reply).toHaveTokenError('BAD_INPUT');
      expect(receipt).toHaveTokenFailure('BAD_INPUT', 'TRANSFER_RESERVES_VALIDATION');
    });

    it("returns success code from _transferReservesFresh and reduces the correct amount", async () => {
      expect(await call(fToken, 'totalReserves')).toEqualNumber(reserves);
      expect(await send(fToken, 'harnessFastForward', [5])).toSucceed();
      expect(await send(fToken, '_transferReserves', [reduction], {from: treasuryGuardian})).toSucceed();
    });
  });
});
