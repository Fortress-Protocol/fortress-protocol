...
describe('FToken', function () {
  let root, accounts;
  let fToken, oldComptroller, newComptroller;
  beforeEach(async () => {
    [root, ...accounts] = saddle.accounts;
    fToken = await makeFToken();
    oldComptroller = fToken.comptroller;
  ...
  describe('_setComptroller', () => {
    it("should fail if called by non-admin", async () => {
      expect(
        await send(fToken, '_setComptroller', [newComptroller._address], { from: accounts[0] })
      ).toHaveTokenFailure('UNAUTHORIZED', 'SET_COMPTROLLER_OWNER_CHECK');
      expect(await call(fToken, 'comptroller')).toEqual(oldComptroller._address);
    });

    it("reverts if passed a contract that doesn't implement isComptroller", async () => {
      await expect(send(fToken, '_setComptroller', [fToken.underlying._address])).rejects.toRevert("revert");
      expect(await call(fToken, 'comptroller')).toEqual(oldComptroller._address);
    });
    ...
    it("reverts if passed a contract that implements isComptroller as false", async () => {
      ...
      await expect(send(fToken, '_setComptroller', [badComptroller._address])).rejects.toRevert("revert marker method returned false");
      expect(await call(fToken, 'comptroller')).toEqual(oldComptroller._address);
    });

    it("updates comptroller and emits log on success", async () => {
      const result = await send(fToken, '_setComptroller', [newComptroller._address]);
      expect(result).toSucceed();
      expect(result).toHaveLog('NewComptroller', {
        oldComptroller: oldComptroller._address,
        newComptroller: newComptroller._address
      });
      expect(await call(fToken, 'comptroller')).toEqual(newComptroller._address);
    });
  });
});
